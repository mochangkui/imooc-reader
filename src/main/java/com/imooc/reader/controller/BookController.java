...
public class BookController {
    ...
    public ModelAndView showDetail (@PathVariable("id") Long id, HttpSession session) {
        ...
        List<Evaluation> evaluationList = evaluationService.selectByBookId(book.getBookId());
        ModelAndView modelAndView = new ModelAndView("/detail");
        Member member = (Member) session.getAttribute("loginMember");
        if (member != null) {
            // 获取会员阅读状态
            MemberReadState memberReadState = memberService.selectMemberReadSate(member.getMemberId(), id);
